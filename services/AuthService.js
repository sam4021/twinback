...
const createStaff = async function(staffInfo){
    ...
    if(validator.isEmail(unique_key)){
        ...
        [err, staff] = await to(Staff.create(staffInfo));
        if(err) TE('staff already exists with that phone number');
...
const authStaff = async function(staffInfo){//returns token
    let unique_key;
    let auth_info = {};
    auth_info.status = 'login';
    unique_key = getUniqueKeyFromBody(staffInfo);
    ...
    if(!staffInfo.password) TE('Please enter a password to login');

    let staff;
    if(validator.isEmail(unique_key)){
        auth_info.method='email';

        [err, staff] = await to(Staff.findOne({where:{email:unique_key}}));
        console.log(err, staff, unique_key);
        if(err) TE(err.message);

    }else if(validator.isMobilePhone(unique_key, 'any')){//checks if only phone number was sent
        auth_info.method='phone';

        [err, staff] = await to(Staff.findOne({where:{phone:unique_key }}));
        if(err) TE(err.message);
    ...
    if(!staff) TE('Not registered');

    [err, staff] = await to(staff.comparePassword(staffInfo.password));
